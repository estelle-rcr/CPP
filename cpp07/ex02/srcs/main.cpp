...
/*   Updated: 2022/04/19 11:37:29 by erecuero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void)
{
	...
	{
		std::cout << "\n>>> TEST ON CONST OBJECT: <<<" << std::endl;
		try {
			int size = 8;
			std::cout << ">> Create an int array of size 8 and select last position" << std::endl;
			const Array<int> intArray2(size);				// const object w/ default initialization at 0
		//	intArray2[7] = 1;								// not assignable but readable
			std::cout << intArray2[7] << std::endl;			// readable
			std::cout << std::endl;
			std::cout << "\n---OK---" << std::endl;
		}
		catch(const std::exception& e) {
